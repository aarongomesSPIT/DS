...
void insertNonFull(BTreeNode *node, Product key)
{
    ...
    else
    {
        ...
        if (node->children[i]->numKeys == MAX_KEYS)
        {
            ...
            if (key.bcode > node->keys[i].bcode)
            {
                i++;
            }
        }
...
void insert(BTreeNode **root, Product key)
{
    ...
    if (r->numKeys == MAX_KEYS)
    {
        ...
        if (newRoot->keys[0].bcode < key.bcode)
        {
            i++;
        }
                  ...
                  Product newKey)
{
    // Check previous key
    if (index > 0 &&
        newKey.bcode <= node->keys[index - 1].bcode)
    {
        return 0;
    }
    // Check next key
            ...
            Product newKey)
{
    ...
    // Quantity-only update: keep the same barcode and change only the stock amount.
    if (oldKey.bcode == newKey.bcode)
    {
        node->keys[index].quantity = newKey.quantity;
